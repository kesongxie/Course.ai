...
class ProfessorSignUpFirstViewController: UIViewController {
    ...
    @IBAction func nextBtnTapped(_ sender: UIButton) {
        if  let fullname = fullNameTextField.text{
            if !fullname.isEmpty{
                if let professorSignUpSecondVC = self.storyboard?.instantiateViewController(withIdentifier: StoryboardIden.ProfessorSignUpSecondViewControllerIden) as? ProfessorSignUpSecondViewController{
                    let selectedUniversityIndex = universityPickerView.selectedRow(inComponent: 0)
                    professorSignUpSecondVC.emailExtension = UniversityDataSource.university[selectedUniversityIndex].emailExtension
                    professorSignUpSecondVC.greetingMessageText = "Prof. " + fullname.components(separatedBy: CharacterSet.whitespacesAndNewlines).first!
                    
                    self.navigationController?.pushViewController(professorSignUpSecondVC, animated: true)
                }
            }
        }
    }
...
extension ProfessorSignUpFirstViewController: UITextFieldDelegate{
...
extension ProfessorSignUpFirstViewController: UIPickerViewDataSource, UIPickerViewDelegate{
